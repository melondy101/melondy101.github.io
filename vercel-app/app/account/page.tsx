...
export default async function AccountPage() {
  ...
  return <main className="shell"><p className="eyebrow">Account</p><h1>{profile?.nickname || user.name || "我的账户"}</h1><p>邮箱：{user.email}</p><p>姓：{profile?.lastName || "尚未填写"}</p><p>账号：{profile?.handle || "尚未填写"}</p><LogoutButton /><h2>收藏文章</h2>{favorites.length ? <ul>{favorites.map(({ slug }) => <li key={slug}><Link href={`/writing/${slug}`}>{slug}</Link></li>)}</ul> : <p>还没有收藏。请在文章详情页收藏。</p>}</main>;
}
