...
export default function ProfileSettingsForm({ profile }: { profile: { lastName: string; nickname: string; handle: string } | null }) {
  ...
  return <form onSubmit={submit}><label>姓<input name="lastName" defaultValue={profile?.lastName} required /></label><label>昵称<input name="nickname" defaultValue={profile?.nickname} required /></label><label>显示账号<input name="handle" defaultValue={profile?.handle} pattern="[a-zA-Z0-9_-]{3,30}" required /></label><button>保存设置</button>{message && <p role="status">{message}</p>}</form>;
}
