...
3. 已登录用户可访问 `/account`，修改姓氏、昵称和显示账号，并查看自己的文章收藏列表；列表中的文章链接返回对应文章详情页。
4. 未登录用户访问账户页或受保护 API 时，被重定向到登录页或收到 `401`。

### 收藏

1. 收藏按钮仅显示在具体文章详情页；文章列表、首页卡片和账户页不提供直接收藏入口。
2. 未登录访客在文章详情页可见“登录后收藏”入口，选择后跳转至登录页，并在登录完成后返回当前文章。
3. 已登录用户在文章详情页选择收藏或取消收藏，并立即获得成功或失败反馈。
4. 服务端从会话取得用户身份，拒绝客户端传入的任意用户 ID。
5. 同一用户对同一文章最多存在一条收藏记录。
